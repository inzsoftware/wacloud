...
func (wac *Conn) Restore() error {
	...
	case r := <-initChan:
		var resp map[string]interface{}
		if err = json.Unmarshal([]byte(r), &resp); err != nil {
			return fmt.Errorf("error1 decoding login connResp: %v\n", err)
		}
	...
	case <-time.After(wac.msgTimeout):
		...
		case r := <-loginChan:
			var resp map[string]interface{}
			if err = json.Unmarshal([]byte(r), &resp); err != nil {
				return fmt.Errorf("error2 decoding login connResp: %v\n", err)
			}
			if int(resp["status"].(float64)) != 200 {
	...
	case r := <-loginChan:
		var resp map[string]interface{}
		if err = json.Unmarshal([]byte(r), &resp); err != nil {
			return fmt.Errorf("error3 decoding login connResp: %v\n", err)
		}
